...
const { buildRentivoListing, hasPrice } = require('../rentivoAPI');
const { RentivoListing } = require('../../models/sequelize');
const { ReservationService } = require('../reservation');
const errors = require('../../util/errors');

class RentivoListingProvider {
  async searchListings({
    ...
    const listings = await RentivoListing.findAll({
      where: {
        isActive: true,
        maxGuests: {
    ...
    if (!checkOutDate && !checkInDate) {
      return sortedListings.filter(listing => hasPrice(listing));
    }
    ...
    return availableListings.filter(listing => hasPrice(listing));
  }
  ...
  async findActiveListing(listingId) {
    ...
    const listing = await RentivoListing.findById(listingId);
    if (!listing) {
  ...
  getListingById(listingId) {
    return buildRentivoListing(listingId);
  }
}

module.exports = { RentivoListingProvider: new RentivoListingProvider() };